                     ...
                     callBack:(RCTResponseSenderBlock)callback
{
    ...
    if ([type isEqualToString:RCTWXShareTypeText]) {
        ...
        NSString * pageUrl = aData[@"webpageUrl"];
        NSString * userName = aData[@"userName"];
        NSString * path = aData[@"path"];
        NSString * hdImageUrl = aData[@"hdImageData"];
        ...
        if (type.length <= 0 || [type isEqualToString:RCTWXShareTypeNews]) {
          ...
          [self.bridge.imageLoader loadImageWithURLRequest:imageRequest callback:^(NSError *error, UIImage *image) {
            if (image == nil){
              ...
              [self shareToWeixinMiniProWithMediaMessage:aScene
                                            Title:title
                                      Description:description
                                       MessageExt:messageExt
                                    MessageAction:messageAction
                                       ThumbImage:aThumbImage
                                         MediaTag:mediaTagName
                                              WebPageUrl:pageUrl
                                                userName:userName
                                                    path:path
                                             hdImageData:UIImagePNGRepresentation(image)
                                         callBack:callback];
...
//分享小程序卡片
- (void)shareToWeixinMiniProWithMediaMessage:(int)aScene
                                Title:(NSString *)title
                          Description:(NSString *)description
                           MessageExt:(NSString *)messageExt
                        MessageAction:(NSString *)action
                           ThumbImage:(UIImage *)thumbImage
                             MediaTag:(NSString *)tagName
                           WebPageUrl:(NSString *)webUrl
                                    userName:(NSString *)userName
                                        path:(NSString *)path
                                 hdImageData:(NSData *)imgeData
                             callBack:(RCTResponseSenderBlock)callback
{
  WXMiniProgramObject *wxMiniObject = [WXMiniProgramObject object];
  wxMiniObject.webpageUrl = webUrl;
  wxMiniObject.userName = userName;
  wxMiniObject.path = path;
//  wxMiniObject.hdImageData = imgeData;
...
RCT_EXPORT_METHOD(shareImagesToTimeline:(NSDictionary *)dic resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
//  NSString *textToShare = [dic stringForKey:@"description"];
  NSString *strImages = [dic stringForKey:@"images"];
  NSArray *arrImages = [strImages componentsSeparatedByString:@"|"];
  NSMutableArray *shareImages = [NSMutableArray array];
  if (arrImages.count)
  {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
      for (NSInteger i = 0; i < arrImages.count; i++) {
        NSString *strImageUrl = [arrImages objectAtIndex:i];
        if ([strImageUrl isKindOfClass:[NSString class]] && strImageUrl.length) {
          ...
          if (!error) {
            ...
            if (image) {
              [shareImages addObject:image];
            }
          }
        }
      }
      
      if (shareImages.count != arrImages.count) {
        NSError *error = nil;
        reject(@"失败", @"失败", error);
      }else
      {
        ...
        UIActivityViewController *activityVC = [[UIActivityViewController alloc]initWithActivityItems:shareImages
                                                                                applicationActivities:nil];
        //不出现在活动项目
        activityVC.excludedActivityTypes = @[UIActivityTypePrint, UIActivityTypeCopyToPasteboard,UIActivityTypeAssignToContact,UIActivityTypeMessage,UIActivityTypeMail,UIActivityTypeSaveToCameraRoll,UIActivityTypeAddToReadingList];
...
//      [shareImages insertObject:GETSTRING(textToShare) atIndex:0];
...
RCT_EXPORT_METHOD(shareImagesToSession:(NSDictionary *)dic resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
  //  NSString *textToShare = [dic stringForKey:@"description"];
  NSString *strImages = [dic stringForKey:@"images"];
