...
import { deviceWidth, isIphoneX, px, deviceHeight, deviceHeightHalf } from "../../utils/Ratio";
import { log } from "../../utils/logs"
import Icon from '../../UI/lib/Icon'
import base from "../../styles/Base";
import GreatBanner from "./GreatBanner"
import GreatQuick from "./GreatQuick"
...
export default class extends React.Component {
    ...
    render() {
        ...
        return <View style={{ flex: 1 }}>
            <View
                style={{ width: deviceWidth, height: this.topHei }}></View>
            <FlatList ref="flatlist"
                ...
                renderItem={({ item, index }) => {
                    if (item.type === 'banner') {
                        return <GreatBanner ref='banner' id={this.props.id} tab={this.props.id}
                            item={item}
                    ...
                    if (item.type === 'quick') {
                        return <GreatQuick ref={e => this.quick = e}
                            onLayout={e => this.onLayout(e, index, item.type)}
                            item={item}
    ...
    topHei = Platform.OS === "ios" ? isIphoneX() ? 132 : px(196) : px(170) + 10;
    //请求banner
    //请求楼层
    //请求时间轴
    //请求商品
    async loadBanner() {
        ...
        try {
            ...
            // let res = require("../../../package/findBannerAndQuickList.json")
            res.type = "banner";
            res.index = "banner";
            res.tt = Date.now();
            this.layout[0] = { h: 0 };
            let quicklist = Object.assign({}, res);
            quicklist.index = "quick";
            quicklist.type = "quick";
            this.layout.push({ h: 0 })

            this.shouldUpdate = true;
            this.setState({ list: [res, quicklist] });
        } catch (e) {
            // toast(e.message);
        } finally {
            this.loading = false
    ...
    _onScroll(e) {
        ...
        const banner_geduan = y < this.layout[0].h - this.topHei - 260;
        
        if (this.lockSwip != banner_geduan) {
            this.lockSwip = banner_geduan;
            this.props.switchLock(this.lockSwip);
        }
    }

    stopScroll() {
        this.refs.flatlist.scrollToOffset({ offset: this.scrollY, animated: false });
    }
    // 精选好货距离屏幕高度
    productTop = 0
    setShopPositionY() {
