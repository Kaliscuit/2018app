...
export default class extends React.Component {
    ...
    async onSubmit() {
        let shopDesc = this.clearSpace(this.state.shopDesc)
        if (!shopDesc) {
            return toast('店铺介绍不能为空');
        }
        if (shopDesc.length > 30) {
            return toast('店铺介绍不能超过30个字符');
        }
        let res = await get(`/shop/updateShopDesc.do?shopDesc=${shopDesc}`);
                ...
                ? <View style={styles.main_box}>
                    <View style={styles.view0}>
                        <TextInput
                            ...
                            placeholder='请输入店铺介绍'
                            onChangeText={res => this.setState({ shopDesc: res })}
                            underlineColorAndroid="transparent">
                        </TextInput>
                    </View>
                    <View style={styles.view1}>
                        <Text allowFontScaling={false} style={styles.sty0}>店铺介绍不能超过30个字符，支持汉字、字母和数字</Text>
                    </View>
