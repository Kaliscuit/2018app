...
export default class extends React.Component {
    ...
    async componentDidMount() {
        try {
            ...
            let bank = [{ bankCode: '', bankName: '请选择开户银行' }].concat(bankList)
    ...
    async onSubmit() {
        let [
            ...
            ], token = this.state.token, mobile = this.state.mobile;
        if (!accountName) {
            toast('姓名不能为空');
            return;
        }
        ...
        try {
            let res = await request.post(`/userBank/saveUserBankWithAliCash.do`, {
                accountName: accountName,
                accountNo: accountNo,
                bankCode: bankCode,
                token: token,
                mobile: mobile
                //?accountName=${accountName}&accountNo=${accountNo}&bankCode=${bankCode}&mobile=${mobile}&verifyCode=${verifyCode}
            });
    ...
    render() {
        const {accountName, accountNo, bankCode} = this.state
        let bankListDom = null;
        if (Platform.OS === 'android') {
            bankListDom = <TouchableOpacity activeOpacity={0.8}>
                <View style={[styles.view1, base.inline]}>
                    <Text allowFontScaling={false} style={styles.sty1}>开户银行</Text>
                    {this.pickerSty([styles.picker1])}
                </View>
            </TouchableOpacity>
        } else {
            bankListDom = <TouchableOpacity activeOpacity={1} onPress={() => {
                this.setState({ pickerStatus: true });
                this.keyBlur();
            }}>
                <View style={[styles.view1, base.inline_left]}>
                    <Text allowFontScaling={false} style={styles.sty1}>开户银行</Text>
                    <Text
                        allowFontScaling={false}
                        style={[styles.bankName, {color: this.state.bankName ? '#252426' : '#b2b3b5'}]}>
                        {this.state.bankName || '请选择开户银行'}
                    </Text>
                    <Icon
                        name={this.state.pickerStatus ? "icon-detail-shousuo" : "icon-detail-zhankai"}
                        style={{width: px(26), height: px(15)}}/>
                </View>
            </TouchableOpacity>
        }
        return <View style={{ flex: 1, backgroundColor: '#f5f3f6' }}>
            ...
            <TouchableOpacity style={[styles.submitBox, {backgroundColor: accountName != '' && bankCode != '' && accountNo != '' ? '#d0648f' : '#b2b3b5'}, base.line]} activeOpacity={0.8} onPress={() => this.onSubmit()}>
