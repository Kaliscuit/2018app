...
export default class PlayVideoPage extends React.Component {
    
    constructor(props){
        super(props);
        const {src} = props.navigation.state.params
        this.state = {
    ...
    render() {
        const {isShowMenu, rotateStatus, fullScreenStatus, src, isEnd, completed} = this.state;
        if (!completed) {
            return (
                <View style={{flex:1, backgroundColor:'#000'}}>
                    {this.renderCenter({videoStatus:true, hint:'视频载入中...'})}
                </View>
            )
        }
        if (!src || src == '') {
            return (
                <View style={{flex:1, backgroundColor:'#000'}}>
                    {this.renderCenter({videoStatus:true, hint:'网络不给力，视频没打开，请点击屏幕退出重试'})}
                </View>
            )
        }
        return (
